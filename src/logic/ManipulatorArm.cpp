...
void ManipulatorArm::rotateJointToTarget(
    int i)
{ // TODO stop after reacing target rotation
    ...
    {
        jointsRotation[i] =
            jointsRotation[i] + std::min(jointRotationSpeed, dRotation);
    }
    else
    {
        jointsRotation[i] =
            jointsRotation[i] - std::min(jointRotationSpeed, dRotationNegative);
    }
}
void ManipulatorArm::resizeSegmentToTarget(
    int i)
{ // TODO stop after reacing target size
...
void ManipulatorArm::moveToTarget()
{ // TODO PID???
    for (int i = 0; i < jointsNumber; i++)
    {
...
void ManipulatorArm::removeBox() { takenBox = nullptr; }
...
void ManipulatorArm::recalculate()
{
    ...
    for (int i = 1; i < jointsNumber; i++)
    {
        lastJointTargetRotation =
            lastJointTargetRotation + jointsTargetRotation[i - 1];
        jointsTargetPosition[i] =
            jointsTargetPosition[i - 1] +
            Vector2d(lastJointTargetRotation, segmentsTargetLength[i - 1]);
    }
    ...
    for (int i = 1; i < jointsNumber; i++)
    {
        ...
        jointsPosition[i] = jointsPosition[i - 1] +
                            Vector2d(lastJointRotation, segmentsLength[i - 1]);
    }
